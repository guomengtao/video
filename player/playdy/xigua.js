...
function OnloadFun()
{
		var checkIEorFirefox = {}
		 if(window.ActiveXObject || window.ActiveXObject !== undefined){
			...
			XfplayIEFF = document.getElementById("xiguaPlayer");
		 }else{
			 isIE = false;
			checkIEorFirefox.firefox = "yes";
			XfplayIEFF = document.getElementById("xiguaPlayer2");
		...
		if(!isIE)
		{
			...
			if(!Install)
			{
				...
				document.getElementById('xframe_mz').src='/player/playdy/xigua.htm';
			}
		}else{
			if(getxfdx('xiguaPlayer2') != null)
				getxfdx('xiguaPlayer2').style.display='none';
