...
dim up_formName,up_formPath,filename,up_file_name,up_fileExt,up_Filesize,up_F_Type
checkPower
Dim isslide,isnews,iszt:isslide=request.QueryString("is")="slide":isnews=request.QueryString("is")="news":iszt=request.QueryString("is")="zt"
if isslide=true then
	up_formPath="../../pic/slide/"
elseif iszt=true then
	up_formPath="../../pic/zt/"
elseif isnews=true then
	sitePic="newspic/"&Year(now())&"-"&Month(now()):up_formPath="../../pic/"&sitePic&"/"
else
	sitePic=sitePic&"/"&Year(now())&"-"&Month(now()):up_formPath="../../pic/"&sitePic&"/"
end if
...
sub upload()
	...
	'<!--7.07-->
		'up_formPath="../../pic/"&sitePic&"/"'upload.form("filepath")
...
Private sub checksave()
	...
	if isslide=true OR iszt=true then
		response.write "<script>try{parent.onUploadBack('"&filename&"')}catch(e){alert('未定义上传回调函数onUploadBack(string filename)')}</script>"
	else
		response.write "<script>parent.document.getElementById(""m_pic"").value='pic/"&sitePic&"/"&filename&"'</script>"
	end if
end Sub
...
Public Function SaveToFile (Path)
  'On Error Resume Next
  createFolder up_formPath,"folderdir"
  Dim oFileStream,s
  s="stream"
