...
Function getSmallMenuInfo(level)
	...
	select case clng(level)
		case 0
			...
			smallMenu(7)="留言管理,admin_gbook.asp?action=comment&where=gbook|回复留言,../gbook.asp|视频评论管理,admin_comment.asp?action=comment&where=1|新闻评论管理,admin_comment.asp?action=comment&where=2|幻灯片管理,admin_expand.asp||马克斯论坛,http://www.maxcms.co||模板下载,http://www.maxcms.co/forum-36-1.html"
			smallMenu(8)="网站配置,admin_config.asp||播放器设置,admin_player1.asp|播放来源管理,admin_player.asp?action=boardsource||系统账号管理,admin_manager.asp"
		case 1
			redim  smallMenu(0)
...
Function getReferedId(str)
	...
	if instr(str,"吉吉影音")>0 then getReferedId = "jjvod": exit function
	...
	if instr(str,"乐视")>0 then getReferedId = "letv": exit function
        if instr(str,"cntv")>0 then getReferedId = "cntv": exit function
	if instr(str,"影音先锋")>0 then getReferedId = "xfplay": exit function
