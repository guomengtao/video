...
AddResUrl 1,"http://api.myzyzy.com/api/max.asp","西瓜影音采集-<font color=""#FF0000"">www.myzyzy.com</font>"
...
Function repairStr(ByVal vstr,ByVal sfrom)
	...
	for i = 0  to ubound(vstr)
                dim xfstr : xfstr = vstr(i)
		if not(isnul(xfstr)) then

                   dim Azf : Azf = split(xfstr,"$$")
                   if(ubound(Azf) >0) then xfstr = Azf(1)

			if regExpObj.Test(xfstr) = false then 
				regExpObj.Pattern = "[\s\S]+?\$[\s\S]+?"
				if regExpObj.Test(xfstr) = true then 
					vstr(i) = trim(xfstr)&"$"&f
				else
					vstr(i) = "第"&j&"集$"&trim(xfstr)&"$"&f
				end if 
...
Sub intoDatabase(url,gtype)
	...
	for i=0 to vNodes.length-1
		...
		for j=0 to l-1
			if vid="" then
				vid=repairStr(childNodes(j).text, "xigua")
                               
			else
				vid= vid & "$$$" & repairStr(childNodes(j).text,"xigua")
			end if
		next
		...
		if not isNul(title) then
			...
			if isNum(localId) then
				...
				if UBound(titleArray)>-1 then
					m_enname=MoviePinYin(titleArray(0))
				else
				...
				if vrsObj.eof then
					sql="insert into {pre}data (m_name,m_type,m_state,m_pic,m_actor,m_des,m_playdata,m_isunion,m_publishyear,m_publisharea,m_note,m_letter,m_enname,m_director,m_lang) values ('"&title&"',"&localId&","&states&",'"&pic&"','"&actor&"','"&des&"','"&vid&"',1,"&publishyear&",'"&publisharea&"','"&m_note&"','"&Left(m_enname,1)&"','"&m_enname&"','"&director&"','"&m_lang&"')"
				else
