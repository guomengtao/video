...
Sub checkRunMode(mode)
	if mode<>"static" then  die "<div style='width:50%;margin-top:50px;background:#66CCCC;font-size:13px;'><br><font color='red'>网站运行模式非静态，不允许生成</font><br><br></div>"
End Sub
...
<!--当前导航-->
<script type="text/JavaScript">if(parent.$('admincpnav')) parent.$('admincpnav').innerHTML='后台首页&nbsp;&raquo;&nbsp;<%=menuList(3,0)%>&nbsp;&raquo;&nbsp;生成选项';</script>
<ul id="makeHtmlTab">
<li id="li.v" onclick="toogletab('0')" style="cursor:pointer">生成视频相关</li>
<li id="li.n" onclick="toogletab('1')" style="cursor:pointer">生成新闻相关</li>
<li id="li.o" onclick="toogletab('2')" style="cursor:pointer">生成其它</li>
</ul>
<table class="tb">
	<tbody id="news" style="display:none;">
		<tr>
		<td><input type="button" value="生成新闻首页" class="btn" onClick="javascript:window.open('?action=newsindex','msg');"<%=ifthen(newsRunMode="static",""," disabled")%>>&nbsp;&nbsp;<input type="button" value="生成地图页" class="btn" onClick="javascript:window.open('?action=newsmap','msg');">&nbsp;<input type="button" class="btn" value="生成JS调用" onClick="javascript:window.open('?action=newsjs','msg');">&nbsp;<input type="button" class="btn" value="一键生成全站" onClick="javascript:window.open('?action=newssite&action3=newssite','msg');"<%=ifthen(newsRunMode="static",""," disabled")%>>&nbsp;<input type="button" class="btn" value="一键生成当天" onClick="javascript:window.open('?action=newsday','msg');"<%=ifthen(newsRunMode="static",""," disabled")%>></td>
		</tr>
		<tr><td>
		...
		&nbsp;<input type="submit" class="btn" value="生成栏目页" onclick="$('newscform').action='?action=newschannel';"<%=ifthen(newsRunMode="static",""," disabled")%>>
		<input type="button" class="btn" value="生成全部栏目页" onClick="javascript:window.open('?action=newsallchannel','msg');"<%=ifthen(newsRunMode="static",""," disabled")%>>
		开始页数：<input type="text" id="startpage" name="startpage" value='1' size="3" maxlength="3"/>结束页数：<input type="text" id="endpage" name="endpage" value='1' size="3" maxlength="6"/>
		<input type="button" class="btn" value="生成部分栏目页" onclick="newssubmitform()"<%=ifthen(newsRunMode="static",""," disabled")%>>
		</form>
		</td></tr>
		<tr><td>
		...
		&nbsp;<input type="submit" class="btn" value="生成内容页" onclick="$('newsviewform').action='?action=newscontent';"<%=ifthen(newsRunMode="static",""," disabled")%>>
		<input type="button" class="btn" value="生成全部内容页" onClick="javascript:window.open('?action=newsallcontent&action2=newsallcontent','msg');"<%=ifthen(newsRunMode="static",""," disabled")%>>
		&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		<input type="button" class="btn" value="生成" onClick="newssubmitvform()"<%=ifthen(newsRunMode="static",""," disabled")%>>
	...
	<tbody id="other" style="display:none;">
		...
		<%echoTopicSelect%>
		<input type="submit" class="btn" value="生成专题分页"<%=ifthen(RunMode="static",""," disabled")%>>
		<input type="button" class="btn" value="生成专题首页列表" onClick="javascript:window.open('?action=topicindex','msg');"<%=ifthen(RunMode="static",""," disabled")%>>
		<input type="button" class="btn" value="一键生成全部专题" onClick="javascript:window.open('?action=alltopic&action2=alltopic&action3=topicindex','msg');"<%=ifthen(RunMode="static",""," disabled")%>>
		</form>
		</td></tr>
		<tr><td>
			...
			<input type="submit" class="btn" value="生成自定义页" >
			<input type="button" class="btn" value="生成全部自定义页" onClick="submitcform()">
			</form>
		</td></tr>
	</tbody>
	<tbody id="video">
		<tr><td><input type="button" value="生成视频首页" class="btn" onClick="javascript:window.open('?action=index','msg');"<%=ifthen(RunMode="static",""," disabled")%>>&nbsp;&nbsp;<input type="button" value="生成地图页" class="btn" onClick="javascript:window.open('?action=map','msg');">&nbsp;<input type="button" value="生成JS调用" class="btn" onClick="javascript:window.open('?action=js','msg');">&nbsp;<input type="button" value="一键生成全站" class="btn" onClick="javascript:window.open('?action=site&action3=site','msg');"<%=ifthen(RunMode="static",""," disabled")%>>&nbsp;<input type="button" value="一键生成当天" class="btn" onClick="javascript:window.open('?action=day','msg');"<%=ifthen(RunMode="static",""," disabled")%>>
		</td>
		</tr>
		<tr><td>
		...
		&nbsp;<input type="submit" class="btn" value="生成栏目页" onclick="$('cform').action='?action=channel';"<%=ifthen(RunMode="static",""," disabled")%>>
		<input type="button" class="btn" value="生成全部栏目页" onClick="javascript:window.open('?action=allchannel','msg');"<%=ifthen(RunMode="static",""," disabled")%>>
		开始页数：<input type="text" id="startpage" name="startpage" value='1' size="3" maxlength="3"/>结束页数：<input type="text" id="endpage" name="endpage" value='1' size="3" maxlength="6"/>
		<input type="button"  class="btn"  value="生成部分栏目页" onclick="submitform()"<%=ifthen(RunMode="static",""," disabled")%>>
		</form>
		</td></tr>
		<tr><td>
		...
		&nbsp;<input type="submit" class="btn" value="生成内容页" onclick="$('viewform').action='?action=content';"<%=ifthen(RunMode="static",""," disabled")%>>
		<input type="button" class="btn" value="生成全部内容页" onClick="javascript:window.open('?action=allcontent&action2=allcontent','msg');"<%=ifthen(RunMode="static",""," disabled")%>>
		&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		<input type="button" class="btn" value="生成" onClick="submitvform()"<%=ifthen(RunMode="static",""," disabled")%>>
		<select name="days" id="days">
		<%
		for i = 1 to 7
		%>
		...
		<br />
		注意：网站在静态模式下，如果<a href="admin_config.asp" target="main"><font color="blue">网站配置</font></a>中选择了<font color="red">"生成播放页"</font>，那么生成内容页的同时、将会生成播放页
		</td></tr>
	</tbody>
</table>
</div>
...
Sub makeSelected
	...
	if m>0 then
		...
		echo "<br>暂停"&makeTimeSpan&"秒后继续生成<script language=""javascript"">setTimeout(""makeNextPage();"","&makeTimeSpan&"000);function makeNextPage(){location.href='?action="&action&"&m_id="&Join(idsArray,",")&"&REFERER="&server.urlencode(back)&"';}</script>"
	else
		alertMsg "",back
	end if
End Sub
...
Sub makeIndex
	...
	echo "首页生成完毕 <a target='_blank' href='/"&sitePath&"index"&fileSuffix&"'><font color=red>浏览首页</font></a><br>" 	
End Sub
...
Sub makeNewsIndex
	...
	createTextFile x,y,""
	echo "首页生成完毕 <a target='_blank' href='"&y&"'><font color=red>浏览新闻首页</font></a><br>" 	
End Sub
...
Sub makeAllmovie
	...
	echo "地图页生成完毕 <a target='_blank' href='/"&sitePath&"allmovie"&fileSuffix&"'><font color=red>浏览地图页</font></a><br>" 	
End Sub
...
Sub makeAllnews
	...
	echo "地图页生成完毕 <a target='_blank' href='"&x&"'><font color=red>浏览新闻地图页</font></a><br>" 	
End Sub
...
Sub makeCustom
	...
	if customtemplate ="" then die "请选择模板"
...
Sub makeCustomInfo(templatename)
	...
	for i=1 to 100
		...
		echo "自定义文件<font color=red>"&Link&"</font>生成完毕 <a target='_blank' href='"&Link&"'><font color=red>浏览页面</font></a><br>"
...
Sub makeVideoJs
	...
	echo "数据JS调用文件生成完毕 <a target='_blank' href='/"&sitePath&"js.js"&"'><font color=red>浏  览</font></a><br>" 
End Sub
...
Sub makeNewsJs
	...
	echo "数据JS调用文件生成完毕 <a target='_blank' href='/"&sitePath&"news.js"&"'><font color=red>浏  览</font></a><br>" 
End Sub

Sub makeTopic
	dim channel:channel = replaceStr(getForm("channel","both")," ",""):curTypeIndex=getForm("index","get"):if isNul(channel) then die "请选择主题"
...
Sub	makeTopicById(Byval topicId)
	...
	if not isArray(topicArray) then die "不存在此专题"
...
Sub makeTopicIndex
	...
	if nCount>=pCount then
		...
		if isNul(action3) then  
			alertMsg "恭喜你，主题列表生成完毕","":terminateAllObjects:die ""
		elseif action3="topicindex" then
			alertMsg "一键生成主题列表全部搞定","":terminateAllObjects:die ""
		end if
	else
...
Sub makeAllTopic
	...
	if isNul(curTypeIndex) then
		...
		if clng(curTypeIndex)>clng(topicIdArrayLen) then
			if isNul(action3) then  
				alertMsg "生成全部主题内页完成","":terminateAllObjects:die ""
			elseif action3="topicindex" then
	...
	topicId = topicIdArray(curTypeIndex)
   	if isNul(topicId) then die "分类丢失" else makeTopicById topicId:echoChannelSuspend curTypeIndex,action3,makeTimeSpan
End Sub
...
Sub makeDay
	...
	if rCount=0 then die  "当天没有视频<br>"
	if isNul(page) then page=0 else page=clng(page)
	echoBegin "正在开始生成当天数据,当前是生成<font color='red'>"&(page+1)&"</font> 到 <font color='red'>"&(page+ifthen(rCount<100,rCount,100))&"</font>页,共<font color='red'>"&rCount&"</font>页<br>","content"
	rsObj.move(page)
	for i=1 to 100
		if not rsObj.eof then
			...
			if ids<>"" then
				echo "生成当天数据搞定<script language='javascript'>self.location='?action=channelbyids&ids="&ids&"&action3=site';</script>":Exit Sub
			else
				alertMsg "一键生成全部搞定","":echoRunTime:terminateAllObjects:die ""
			end if
		end if
	next
...
Sub makenewsDay
	...
	echoBegin "正在开始生成当天数据,当前是第<font color='red'>"&page&"</font>页,共<font color='red'>"&rsObj.pagecount&"</font>页<br>","content"
	for i=1 to rsObj.pagesize
		...
		if rsObj.eof  then
			...
			if ids<>"" then
				echo "生成当天数据搞定<script language='javascript'>self.location='?action=newschannelbyids&ids="&ids&"&action3=newssite';</script>":Exit Sub
			else
				alertMsg "一键生成全部搞定","":echoRunTime:terminateAllObjects:die ""
			end if
		end if
...
Sub echoDaySuspend(curPage,days,sleep)
	echo "<br>暂停"&sleep&"秒后继续生成<script language=""javascript"">setTimeout(""makeNextPage();"","&sleep&"000);function makeNextPage(){location.href='?action="&action&"&page="&curPage&"&days="&days&"';}</script>"
End Sub
...
Sub makeAllContent
	...
	if isNul(curTypeIndex) then 
		...
		if clng(curTypeIndex)>clng(typeIdArrayLen) then
			if isNul(action3) then  
				alertMsg "生成全部内容页完成","":terminateAllObjects:die ""
	...
	typeId = typeIdArray(curTypeIndex)
   	if isNul(typeId) then die "分类丢失" else makeContentByChannel typeId,false
End Sub
...
Sub makeAllArticle
	...
	typeId = typeIdArray(curTypeIndex)
   	if isNul(typeId) then die "分类丢失" else makeArticleByChannel typeId,false
End Sub

Sub makeContent
	dim channel:channel = getForm("channel","both"):if isNul(channel) then die "请选择分类"
...
Sub makeContentByChannel(channel,isIncludeSub)
	...
	if isTypeHide(channel)=true or rCount=0 then
		if isNul(action2) then
			echo "该分类<font color='red'>"&typeZnName&"</font>无视频或被隐藏<br>":Exit Sub
		elseif action2="allcontent" then
			echo "该分类<font color='red'>"&typeZnName&"</font>无视频或被隐藏<br>":echoContentSuspendPerChannel curTypeIndex+1,action2,action3,ifthen(isMakePlay=3,"0.5",makeTimeSpan):Exit Sub
		end if
	end if
	if isNul(currentPage) then currentPage=0 else currentPage=clng(currentPage)
	echoBegin "正在开始生成栏目<font color='red'>"&typeZnName&"</font>的内容页,当前是生成<font color='red'>"&(currentPage+1)&"</font> 到 <font color='red'>"&(currentPage+ifthen(rCount<100,rCount,100))&"</font>页,共<font color='red'>"&rCount&"</font>页<br>","content"
	...
	for i=1 to 100
		if not rsObj.eof then
			...
			if isNul(action2) then
				echo "恭喜此分类搞定":Exit Sub
			elseif action2="allcontent" then
				echo "恭喜此分类搞定":echoContentSuspendPerChannel curTypeIndex+1,action2,action3,makeTimeSpan:Exit Sub
			end if
		end if
	next
...
Sub makeArticleByChannel(channel,isIncludeSub)
	...
	if isNewsTypeHide(channel)=true or rsObj.recordcount=0 then
		if isNul(action2) then 
			echo "该分类<font color='red'>"&typeZnName&"</font>无新闻或被隐藏<br>":Exit Sub
		elseif action2="newsallcontent" then
			echo "该分类<font color='red'>"&typeZnName&"</font>无新闻或被隐藏<br>":echoContentSuspendPerChannel curTypeIndex+1,action2,action3,newsmakeTimeSpan:Exit Sub
		end if	
	end if
	rsObj.pagesize=100
	rsObj.absolutepage=currentPage
	echoBegin "正在开始生成栏目<font color='red'>"&typeZnName&"</font>的内容页,当前是第<font color='red'>"&currentPage&"</font>页,共<font color='red'>"&rsObj.pagecount&"</font>页<br>","content"
	for i=1 to rsObj.pagesize
		makeArticleById(rsObj(0))
		rsObj.movenext
		if rsObj.eof then
			if  isNul(action2) then
				echo "恭喜此分类搞定":Exit Sub
			elseif action2="newsallcontent" then
				echo "恭喜此分类搞定":echoContentSuspendPerChannel curTypeIndex+1,action2,action3,newsmakeTimeSpan:Exit Sub
			end if
		end if
	next
...
Sub echoContentSuspend(typeIdIndex,page,channel,action2,action3,sleep)
	echo "<br>暂停"&sleep&"秒后继续生成<script language=""javascript"">setTimeout(""makeNextPage();"","&sleep&"000);function makeNextPage(){location.href='?action="&action&"&index="&typeIdIndex&"&channel="&channel&"&page="&page&"&action2="&action2&"&action3="&action3&"';}</script>"
End Sub

Sub echoContentSuspendPerChannel(typeIdIndex,action2,action3,sleep)
	echo "<br>暂停"&sleep&"秒后继续生成<script language=""javascript"">setTimeout(""makeNextType();"","&sleep&"000);function makeNextType(){location.href='?action="&action&"&index="&typeIdIndex&"&action2="&action2&"&action3="&action3&"';}</script>"
End Sub
...
Sub makeContentById(Byval vId)
	...
	if rsObj.recordcount=0 then echo "<font color='red'>数据ID:"&vId&" 该数据所属分类被隐藏，跳过生成</font><br>":Exit Sub
	if rsObj("m_recycle")=1 then echo "<font color='red'>数据ID:"&vId&" 该数据所属分类被隐藏，跳过生成</font><br>":Exit Sub
	...
	for n=playn to 1
		...
		select case typeFlag
			...
			case "parse_play_"
				...
				playORdownData=replaceStr(rsObj("m_playdata"),"播客CC","FLV高清"):y="/"&sitepath&"playdata/"&(vId mod 256)&"/"&vId&".js"
...
Sub makeArticleById(Byval vId)
	...
	if rsObj.recordcount=0 then
		rsObj.close:set rsObj = nothing:echo "<font color='red'>该新闻被删除或隐藏</font><br>":exit sub
	end if
	vType=rsObj("m_type"):venname=rsObj("m_entitle"):vdatetime=rsObj("m_datetime")
	if InStr(" ,"&getHideNewsTypeIDS()&",",","&vType&",")>0 then
		rsObj.close:set rsObj = nothing:echo "<font color='red'>数据ID:"&vId&" 该数据所属分类被隐藏，跳过生成</font><br>":exit sub
	end if
...
Sub makeChannelByIDS
	...
	if isNul(curTypeIndex) then
		...
		if clng(curTypeIndex)>clng(typeIdArrayLen) then
			if isNul(action3) then
				alertMsg "生成所有栏目全部搞定","":terminateAllObjects:die ""
			elseif action3="site" then
				makeIndex:makeAllmovie:alertMsg "一键生成全部搞定","":terminateAllObjects:die ""
			end if
		end if
	end if
	typeId = typeIdArray(curTypeIndex)
   	if isNul(typeId) then die "分类丢失" else makeChannelById typeId:echoChannelSuspend2 ids,curTypeIndex,action3,makeTimeSpan
End Sub
	...
	if isNul(curTypeIndex) then
		...
		if clng(curTypeIndex)>clng(typeIdArrayLen) then
			if isNul(action3) then
				alertMsg "生成所有栏目全部搞定","":terminateAllObjects:die ""
			elseif action3="newssite" then
				makeNewsIndex:makeAllnews:alertMsg "一键生成全部搞定","":terminateAllObjects:die ""
			end if
		end if
	end if
	typeId = typeIdArray(curTypeIndex)
   	if isNul(typeId) then die "分类丢失" else makeNewsPartById typeId:echoChannelSuspend2 ids,curTypeIndex,action3,newsmakeTimeSpan
End Sub

Sub echoChannelSuspend2(ids,typeIdIndex,action3,sleep)
	echo "<br>暂停"&makeTimeSpan&"秒后继续生成<script language=""javascript"">setTimeout(""makeNextChannel();"","&makeTimeSpan&"000);function makeNextChannel(){location.href='?action="&action&"&ids="&ids&"&index="&typeIdIndex&"&action3="&action3&"&page="&currentPage&"';}</script>"
End Sub
...
Sub makeAllChannel
	...
	typeId = typeIdArray(curTypeIndex)
   	if isNul(typeId) then die "分类丢失" else makeChannelById typeId:echoChannelSuspend curTypeIndex,action3,makeTimeSpan
End Sub
...
Sub makeAllNewsPart
	...
	typeId = typeIdArray(curTypeIndex)
   	if isNul(typeId) then die "分类丢失" else makeNewsPartById typeId:echoChannelSuspend curTypeIndex,action3,newsmakeTimeSpan
End Sub

Sub echoChannelSuspend(Index,action3,sleep)
	echo "<br>暂停"&sleep&"秒后继续生成<script language=""javascript"">setTimeout(""makeNextChannel();"","&sleep&"000);function makeNextChannel(){location.href='?action="&action&"&index="&Index&"&action3="&action3&"&page="&currentPage&"';}</script>"
End Sub
...
Sub makeChannel
	dim channel:channel = replaceStr(getForm("channel","both")," ",""):curTypeIndex=getForm("index","get"):if isNul(channel) then die "请选择分类"
	if isNum(curTypeIndex) then curTypeIndex=Clng(curTypeIndex) else curTypeIndex=0
...
Sub echoChannelSuspend3(channel,typeIdIndex,sleep)
	echo "<br>暂停"&sleep&"秒后继续生成<script language=""javascript"">setTimeout(""makeNextChannel();"","&sleep&"000);function makeNextChannel(){location.href='?action="&action&"&channel="&channel&"&index="&typeIdIndex&"&page="&currentPage&"';}</script>"
End Sub

Sub makeLengthchannel
	dim channel:channel = replaceStr(getForm("channel","both")," ",""):if isNul(channel) then die "请选择分类"
	...
	if startpage<=0 then die "您输入的页数小于0"
	if startPage>endpage then die "您输入的结束页小于开始页"
...
Sub	makeChannelById(Byval typeId)
	...
	tempStr = templateObj.content:pCount = rsObj.pagecount
	if isTypeHide(typeId)=1 then echo "分类<font color=red >"&nameAndTemplateArray(0)&"</font>为隐藏状态、跳过生成":rsObj.close:set rsObj = nothing:exit sub
	if rsObj.recordcount=0 then
...
Sub	makeNewsPartById(Byval typeId)
	...
	tempStr = templateObj.content:pCount = rsObj.pagecount
	if isNewsTypeHide(typeId)=1 then echo "分类<font color=red >"&nameAndTemplateArray(0)&"</font>为隐藏状态、跳过生成":rsObj.close:set rsObj = nothing:exit sub
	if rsObj.recordcount=0 then
...
Sub	makeLengthChannelById(Byval typeId,byval startpage ,byval endPage)
	...
	echoBegin nameAndTemplateArray(0),"channel"
	set rsObj = conn.db("select m_id,m_name,m_type from {pre}data where m_recycle=0 AND m_type in ("&typeIds&")","records1")
	if rsObj.recordcount=0 then echo "<font color='red'>该目录无数据</font><br>":Exit Sub
	rsObj.pagesize = pSize:currentTypeId = typeId:cacheName = "parse_channel_"&currentTypeId
	if cacheStart=1 then 
		if cacheObj.chkCache(cacheName) then 
			templateObj.content = cacheObj.getCache(cacheName)  
...
Sub echoBegin(Byval str,Byval pType)
	select case pType
		case "channel","newspage"
			echo "正在开始生成栏目<font color='red'>"&str&"</font>的列表<br>"
		case "content"
			echo str
		case "topicindex"
			echo "正在开始生成专题主列表<br>" 
		case "topicpage"
			echo "正在开始生成专题<font color='red'>"&str&"</font>的列表<br>"
	end select
End Sub
...
Sub echoEach(Byval str,Byval cur,Byval link,Byval pType)
	select case pType
		...
		case "play"
			echo " &nbsp;&nbsp;成功生成<font color='blue'>"&str&"</font>播放地址：<a href='"&link&"' target='_blank'><font color='blue'>"&link&"</font></a><br>"
		case "news"
			echo " 成功生成新闻<font color='red'>"&str&"</font>的地址：<a href='"&link&"' target='_blank'><font color='red'>"&link&"</font></a><br>"
		case "topicindex"
			echo " 成功生成主题列表<font color='red'>"&str&"</font>的地址：<a href='"&link&"' target='_blank'><font color='red'>"&link&"</font></a><br>"
		case "topicpage"
			echo " 成功生成主题内页<font color='red'>"&str&"</font>的地址：<a href='"&link&"' target='_blank'><font color='red'>"&link&"</font></a><br>"
		end select
End Sub
...
Sub echoTopicSelect
	...
	echo makeTopicSelect("channel",topicArray,"请选择专题","")
End Sub

Sub makeBaidu
	if flag<>1 then
		echo "<div><b>生成baidu地图</b>： 总输出数量<input type='text' id='allmakenum' value='500'>每页数量<input type='text' id='makenum' value='100'> <input type='button' class='btn' value='开始生成' onclick=""javascript:location.href='?action=baidu&flag=1&allmakenum='+$('allmakenum').value+'&makenum='+$('makenum').value"" /></div>"
	else
		...
		for i=1 to pagenum
			...
			echo "http://"&siteUrl&"/"&sitePath&"baidu"&xmlUrl&".xml"&" 生成完毕 <a target='_blank' href='../baidu"&xmlUrl&".xml'><font color=red>浏览</font></a><br>"
		next
		rsObj.close:set rsObj=nothing:echo "生成完毕"
		die  "请通过<a href='http://news.baidu.com/newsop.html' target='_blank'>http://news.baidu.com/newsop.html</a>提交!"
	end if	
End Sub

Sub makeGoogle
	if flag<>1 then
		echo "<div><b>生成google地图</b>： 总输出数量<input type='text' id='allmakenum' value='500'>每页数量<input type='text' id='makenum' value='100'> <input type='button' class='btn' value='开始生成' onclick=""javascript:location.href='?action=google&flag=1&allmakenum='+$('allmakenum').value+'&makenum='+$('makenum').value""/></div>"
	else
		if isNul(makenum) then makenum=100 else makenum=clng(makenum)
		allmakenum=getForm("allmakenum","get"):if isNul(allmakenum) then allmakenum=500 else allmakenum=clng(allmakenum)
		...
		for i=1 to pagenum
			...
			echo "http://"&siteUrl&"/"&sitePath&"google"&xmlUrl&".xml"&" 生成完毕 <a target='_blank' href='../google"&xmlUrl&".xml'><font color=red>浏览</font></a><br>"
		next
		rsObj.close:set rsObj=nothing:echo "生成完毕"
		die  "请通过<a href='http://www.google.com/webmasters/tools/' target='_blank'>http://www.google.com/webmasters/tools/</a>提交!"
	end if	
End Sub
...
Sub makeDaysview
	...
	for i=1 to 100
		if not rsObj.eof then
			...
			echo "生成最近"&days&"天搞定":rsObj.close:set rsObj = nothing:exit sub
		end if
	next
...
Sub makeRss
	if flag<>1 then
		echo "<div><b>生成RSS地图</b>： 输出数量<input type='text' id='makenum' value='100'> <input type='button' class='btn' value='开始生成' onclick=""javascript:location.href='?action=rss&flag=1&makenum='+$('makenum').value"" /></div>"
	else
		if isNul(makenum) then makenum=100 else makenum=clng(makenum)
		...
		echo "http://"&siteUrl&"/"&sitePath&"rss.xml <a target='_blank' href='../rss.xml'><font color=red>浏览</font></a><br>"
	end if	
End Sub
