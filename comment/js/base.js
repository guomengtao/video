...
function show(data){
	...
	var htmlstr=[],uname,ctxt,ctime,allowreply,pagenum_s,pagenum_e;
	...
	if(pobj.count){
		...
		htmlstr.push("<div class=\"pager\">");
		if(pobj.page != 1)htmlstr.push("<a href=\"#\" onclick=\"return page("+param.type+","+param.gid+",1);\">&#8249;&#8249; 第一页</a><a href=\"#\" onclick=\"return page("+param.type+","+param.gid+","+(pobj.page-1)+");\">&#8249; 上一页</a>");
		...
		for(var z = pagenum_s;z<=(pobj.count>pagenum_e?pagenum_e:pobj.count);z++){
			if(z==pobj.page)htmlstr.push("<span>"+z+"</span>");
			else htmlstr.push("<a href=\"#\" onclick=\"return page("+param.type+","+param.gid+","+z+");\">"+z+"</a>");
		}
		//if(pagenum_e<pobj.count)htmlstr.push("<a href=\"#\" onclick=\"page("+param.type+","+param.gid+","+(pagenum_e+1)+");\" class=\"non\">...</a>");
		if(pobj.page !== pobj.count)htmlstr.push("<a href=\"#\" onclick=\"return page("+param.type+","+param.gid+","+(pobj.page+1)+");\">下一页 &#8250;</a><a href=\"#\" onclick=\"return page("+param.type+","+param.gid+","+pobj.count+");\">最后页 &#8250;&#8250;</a>");
		htmlstr.push("</div>");
	}
	//write
...
function getcaptcha(){
	var uri = "captcha/getcode.asp?r="+Math.random();
	$("#gcaptcha").val("");
	$('#gcaptcha').focus();
	$("#getcode").css('display','inline');
	$("#codeimg").html("<img src=\""+uri+"\" width=\"46\" height=\"18\" align=\"top\"/>");
	return false;
}
//submit
function submitform(){
	...
	var pic = document.getElementById("pfile") && $("#pfile").val().replace(/[\t\s]+/g,"");
	if(pic && user.upfile==""){
		$("#form1").attr("action","api/send.asp?action=1&captcha="+captchastr);
		$("#submit1").attr("value","正在提交,请稍侯...");
		$("#submit1").attr("disabled","disabled");
		$("#form1").el.submit();
	}
	else submit_2(user.upfile || "");
}
